/* ===== EconoLearn – main.jsx (stable UI + floating theme + quote) ===== */
...
/* ---------- Micro-UI helpers ---------- */
const glassCard = "relative overflow-visible rounded-3xl p-6 bg-white/60 backdrop-blur-xl border border-white/50 shadow-[0_10px_40px_-10px_rgba(239,156,167,.45)] dark:bg-[#0b1220]/70 dark:border-white/10";
const cardWrap  = "relative rounded-3xl p-[1px] bg-gradient-to-br from-rose-100 via-rose-50 to-rose-100 dark:from-[#111827] dark:via-[#0f172a] dark:to-[#111827]";
const glassBtn  = "ripple touch-press px-4 py-2 rounded-lg border border-white/60 bg-white/70 hover:bg-white text-gray-800 backdrop-blur transition shadow-sm hover:shadow hover:-translate-y-[1px] dark:bg-white/10 dark:text-gray-100 dark:border-white/10 dark:hover:bg-white/15";
...
(function injectRippleCSS(){
  ...
  style.textContent = `
    ...
    :root.deep{color-scheme:dark}
  `;
  document.head.appendChild(style);
})();

/* ================= THEME (isolated, floating dock) ================= */
...
function useTheme(){
  const [theme,setTheme] = React.useState(localStorage.getItem(THEME_KEY) || 'system');
  React.useEffect(()=>{ applyTheme(theme); localStorage.setItem(THEME_KEY, theme); },[theme]);
  React.useEffect(()=>{
...
const ThemeDock = ({theme,setTheme}) => (
  <div className="fixed right-4 bottom-4 z-50 flex items-center gap-2 p-2 rounded-2xl border shadow-xl
                  bg-white/90 backdrop-blur dark:bg-[#101827]/90 dark:border-white/10">
    {['light','dark','deep','system'].map(mode=>(
      <button key={mode}
        className={`px-3 py-1 rounded-full text-sm border transition ${
          theme===mode ? 'bg-teal-600 text-white border-teal-600 shadow'
                       : 'bg-transparent border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10 text-gray-800 dark:text-gray-100'
        }`}
...
/* ---------------- Quote card (home only, non-intrusive) ------------- */
const QuoteCard = () => {
  const QUOTES = useMemo(()=>[
    { text: "Growth without equity does not endure.", author: "Amartya Sen (paraphrased)" },
    { text: "Incentives matter — often more than we think.", author: "Paul Krugman" },
    { text: "Good policy is economics made practical.", author: "Abhijit Banerjee (paraphrased)" },
    { text: "Ideas drive progress when paired with evidence.", author: "Esther Duflo" }
  ],[]);
  const q = useMemo(()=>QUOTES[Math.floor(Math.random()*QUOTES.length)],[]);
  return (
    <section className="max-w-5xl mx-auto px-4">
      <div className="rounded-2xl px-6 py-5 border shadow-[0_20px_60px_rgba(0,0,0,.12)]
                      bg-white/90 backdrop-blur dark:bg-[#101827]/90 dark:border-white/10">
        <div className="flex items-start gap-3">
          <div className="text-4xl leading-none text-rose-400 select-none">“</div>
          <div className="flex-1">
            <p className="font-serif text-xl leading-relaxed">{q.text}</p>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-300 italic">— {q.author}</p>
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              <span className="px-3 py-1 rounded-full bg-rose-50 text-rose-700 border border-rose-100 dark:bg-white/5 dark:text-rose-200 dark:border-white/10">
                Crafted with 💗 for exam excellence
              </span>
              <span className="px-3 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-100 dark:bg-white/5 dark:text-teal-200 dark:border-white/10">
                Built by Shailesh Kumar + GPT-5
              </span>
            </div>
          </div>
        </div>
...
const Header = ({page,onHome,onHistory,onAnalytics}) => (
  <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b dark:bg-[#0b1220]/80 dark:border-white/10">
...
const NativeSelect = ({value,onChange,options}) => (
  <select value={value} onChange={e=>onChange(e.target.value)}
          className="ripple w-full p-2 pr-9 border rounded-lg bg-white/70 backdrop-blur hover:bg-white transition dark:bg-white/10 dark:text-gray-100 dark:border-white/10 dark:hover:bg-white/15">
...
const FancySelect = ({value,onChange,options}) => {
  ...
  return (
    <div className="relative">
      <button ref={ref} type="button" className="ripple w-full text-left p-2 pr-9 border rounded-lg bg-white/70 hover:bg-white transition dark:bg-white/10 dark:text-gray-100 dark:border-white/10 dark:hover:bg-white/15"
              onClick={()=>setOpen(v=>!v)}>
        {value}<span className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500">▾</span>
      </button>
      {open&&(
        <ul ref={list} className="absolute z-30 left-0 right-0 max-h-60 overflow-auto rounded-xl border bg-white/95 backdrop-blur shadow-xl mt-2 dark:bg-[#0b1220]/95 dark:text-gray-100 dark:border-white/10">
          {options.map(opt=>(
            <li key={opt} onClick={()=>{onChange(opt); setOpen(false);}}
                className={`px-3 py-2 cursor-pointer hover:bg-teal-50 dark:hover:bg-white/10 ${opt===value?'bg-teal-100 text-teal-700 font-medium dark:bg-white/10 dark:text-teal-200':''}`}>
              {opt}
            </li>
          ))}
        </ul>
...
/* ================================ APP ================================== */
const App = () => {
  const [theme,setTheme] = useTheme();        // NEW: floating theme
  ...
  if(loading) return (
    <>
      <Header page={page}/>
      <main className="max-w-6xl mx-auto px-4 py-10 text-center text-gray-500 dark:text-gray-300">Loading…</main>
                  ...
                  <div className="flex items-end gap-4">
                    <div>
                      ...
                      <input type="number" min="1" max={filteredCount} value={testCount}
                            ...
                            className="ripple w-32 p-2 border rounded-lg bg-white/70 dark:bg-white/10 dark:text-gray-100 dark:border-white/10"/>
                      <p className="text-xs text-gray-700 dark:text-gray-300 mt-1">
                        Available: {filteredCount}{req>filteredCount && <span className="ml-2 text-rose-600 dark:text-rose-300">(Requested {req}, using {effectiveN})</span>}
                      </p>
                    </div>
                    <div>
                      <label className="text-sm block">Time limit</label>
                      <div className="p-2 border rounded bg-white/70 text-sm w-32 text-center dark:bg-white/10 dark:text-gray-100 dark:border-white/10">{fmt(est)}</div>
                    </div>
                  </div>
                </div>
              )}
  ...
  if(page==='quiz'){
    ...
    const markClass = isMarked
      ...
      : "bg-white/70 text-gray-800 border-white/60 hover:bg-white dark:bg-white/10 dark:text-gray-100 dark:border-white/10 dark:hover:bg-white/15";
        ...
        <main className="max-w-6xl mx-auto px-4 py-6">
          <div className="grid lg:grid-cols-[1fr,280px] gap-6">
            <div>
              ...
              <section className={cardWrap}>
                <div className={glassCard + " animate-[slide_.35s_ease_both]"}>
                  <div className="absolute right-4 top-4 text-xs text-gray-700 dark:text-gray-300 bg-white/70 border border-white/60 rounded-md px-2 py-1 dark:bg-white/10 dark:border-white/10">
                      ...
                      return (
                        <label key={idx}
                          className={`ripple touch-press flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition
                                      ...
                                      bg-white/70 backdrop-blur hover:bg-white dark:bg-white/10 dark:text-gray-100 dark:border-white/10 dark:hover:bg-white/15 ${active?'border-teal-500 ring-1 ring-teal-300':''}`}>
            ...
            <aside className="lg:sticky lg:top-[72px]">
              <div className="rounded-2xl p-4 bg-white/70 border border-white/60 shadow dark:bg-white/10 dark:text-gray-100 dark:border-white/10">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold">Question Palette</h4>
                  {mode==='test' && <span className={`text-xs px-2 py-1 rounded border ${remaining<=30?'border-red-500 text-red-600':'border-gray-300 text-gray-700 dark:border-white/10 dark:text-gray-100'}`}>⏱ {fmt(remaining)}</span>}
                </div>
                  ...
                  {activeSet.map((_,i)=>{
                    ...
                    const color = s==='attempted_marked' ? "bg-blue-500 text-white border-blue-600 hover:brightness-95"
                                 ...
                                 : s==='attempted'       ? "bg-[#32CD32] text-white border-green-600 hover:brightness-95"
                                                         : "bg-white text-gray-800 border-gray-300 hover:bg-gray-100 dark:bg-white/10 dark:text-gray-100 dark:border-white/10";
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-xs mt-3 text-gray-700 dark:text-gray-300">
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-white border border-gray-300 dark:bg-white/10 dark:border-white/10"></span>Unattempted</div>
          ...
          <section className={cardWrap}><div className={glassCard}>
            ...
            <div className="space-y-3 mt-4">
              {activeSet.map((qq,i)=>{
                ...
                return (
                  <div key={i} className="p-3 border rounded bg-white/70 dark:bg-white/10 dark:text-gray-100 dark:border-white/10">
                    <div className="flex justify-between">
                      <b>Q{i+1}. {qq.question}</b>
                      <span className={`text-xs px-2 py-1 rounded ${ok?'bg-green-100 text-green-700':'bg-red-100 text-red-700'} dark:bg-white/10 dark:text-gray-100`}>{ok?'Correct':'Incorrect'}</span>
                    </div>
      ...
      <>
        ...
        <main className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <select className="ripple border rounded px-2 py-1 bg-white/70 backdrop-blur hover:bg-white dark:bg-white/10 dark:text-gray-100 dark:border-white/10 dark:hover:bg-white/15"
          ...
          {sorted.length===0 ? (
            ...
            <div className="space-y-4">
              {sorted.map(a=>(
                <details key={a.id} className="rounded-xl border bg-white/70 backdrop-blur dark:bg-white/10 dark:text-gray-100 dark:border-white/10">
                  <summary className="ripple cursor-pointer flex items-center justify-between px-4 py-3 border-b hover:bg-white dark:hover:bg-white/5">
                    <div>
          ...
          <section className={cardWrap}><div className={glassCard}>
            ...
            {rows.length===0 ? <div className="text-gray-500 dark:text-gray-300">No data yet.</div> : (
              <div className="space-y-3">
                {rows.map(r=>(
                  <div key={r.ch} className="p-3 border rounded-xl bg-white/70 dark:bg-white/10 dark:text-gray-100 dark:border-white/10">
                    ...
                    <div className="mt-2 h-3 w-full bg-gray-100 rounded-full overflow-hidden dark:bg-white/10">
